...
extern "C" {
  ...
  std::string get_calc(pb::event_command::ptr e, std::string name) {
  ...
  bool has_calc(pb::event_command::ptr e, std::string name) {
    pb::db_service::ptr db_s = pb_plugin->dm->get<pb::db_service>();
  ...
  void update_calc(pb::event_command::ptr e, std::string name, std::string value) {
    pb::db_service::ptr db_s = pb_plugin->dm->get<pb::db_service>();
    ...
    int ret;
  pbc_update_calc_step:
    ret = sqlite3_step(stmt);
    if (ret == SQLITE_ROW) goto pbc_update_calc_step;
    if (ret == SQLITE_ERROR) {
      throw std::runtime_error(std::string(sqlite3_errmsg(db_s->get_db())));
      sqlite3_finalize(stmt);
    }
  ...
  void insert_calc(pb::event_command::ptr e, std::string name, std::string value) {
    pb::db_service::ptr db_s = pb_plugin->dm->get<pb::db_service>();
    ...
    int ret;
  pbc_insert_calc_step:
    ret = sqlite3_step(stmt);
    if (ret == SQLITE_ROW) goto pbc_insert_calc_step;
    if (ret == SQLITE_ERROR) {
      throw std::runtime_error(std::string(sqlite3_errmsg(db_s->get_db())));
      sqlite3_finalize(stmt);
    }
  ...
  void set_calc(pb::event_command::ptr e, std::string name, std::string value) {
    pb::db_service::ptr db_s = pb_plugin->dm->get<pb::db_service>();
    if (has_calc(e, name)) {
  ...
  void clear_calc(pb::event_command::ptr e, std::string name) {
    pb::db_service::ptr db_s = pb_plugin->dm->get<pb::db_service>();
  ...
  pbc_clear_calc_step:
    ret = sqlite3_step(stmt);
    if (ret == SQLITE_ROW) goto pbc_clear_calc_step;
    if (ret == SQLITE_ERROR) {
      throw std::runtime_error(std::string(sqlite3_errmsg(db_s->get_db())));
      sqlite3_finalize(stmt);   
    }
  ...
  void pb_init(std::vector<std::pair<std::string, std::string>>& cfg) {    
    ...
    pb::command calc_cmd {
      ...
      .handler     = [] (pb::command& c, pb::event_command::ptr e) {
        if (e->split.size() == 2) {
          e->socket->stream() << pb::ircstream::reply(e, e->split[1] + " = " + get_calc(e, e->split[1]));
    ...
    pb_plugin->register_command(calc_cmd);
  }
